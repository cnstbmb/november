...
describe("DoublyLinkedList", () => {
  ...
  it("should be possible to store objects in the list and to print them out", () => {
    ...
    const nodeStringifier = (value: { key: any; value: any; }) => `${value.key}:${value.value}`;
  ...
  it("should find node by callback", () => {
    ...
    const node = linkedList.find(undefined, (value: { key: string; }) => value.key === "test2");

    expect(node).toBeDefined();
    expect(node?.value.value).toBe(2);
    expect(node?.value.key).toBe("test2");
    expect(
      linkedList.find(undefined, (value: { key: string; }) => value.key === "test5")
    ).toBeNull();
  });

  it("should find node by means of custom compare function", () => {
    const comparatorFunction = (a: { customValue: number; }, b: { customValue: number; }) => {
