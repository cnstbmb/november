...
- `infra/ansible-template/inventories/prod/hosts.yml`
- `infra/ansible-template/inventories/prod/group_vars/all.yml`
- `infra/ansible-template/inventories/prod/group_vars/master.yml` (optional overrides)
- `infra/ansible-template/inventories/prod/group_vars/workers.yml` (optional overrides)
...
If you keep them in Docker, set these variables in `infra/ansible-template/inventories/prod/group_vars/master.yml`:
...
``` bash
ansible-playbook -i infra/ansible-template/inventories/prod/hosts.yml infra/ansible-template/playbooks/site.yml
```
...
BRANCH="main" \
sudo sh infra/ansible-template/prepare_remote.sh
```
...
- `infra/ansible-template/inventories/prod/hosts.yml`
- `infra/ansible-template/inventories/prod/group_vars/all.yml`
- `infra/ansible-template/inventories/prod/group_vars/master.yml`
- `infra/ansible-template/inventories/prod/group_vars/workers.yml`

4) Run with validation:

``` bash
REPO_DIR="/opt/november" \
INVENTORY_PATH="infra/ansible-template/inventories/prod/hosts.yml" \
PLAYBOOK_PATH="infra/ansible-template/playbooks/site.yml" \
sh infra/ansible-template/run_playbooks.sh
```

Notes:

- The prepare step installs `ansible` and `git` on the master and pulls your repo.
- It uses `git sparse-checkout` to fetch only `infra/ansible-template/` and `deployments/prod`.
...
``` bash
ansible-playbook -i infra/ansible-template/inventories/prod/hosts.yml infra/ansible-template/playbooks/master.yml
ansible-playbook -i infra/ansible-template/inventories/prod/hosts.yml infra/ansible-template/playbooks/workers.yml
```
